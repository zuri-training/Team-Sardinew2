...
exports.createForm = async (req, res) => {
  try {
    ...
    if(!(userId && title && image && details && isResponseOnce )){ //validate user input
      return res.status(400).json({success:false, message:"All input is required!"});
    }

    const result = await Form.create({   //save Form in the DB
      userId,
      title,
      image,
      details,
...
exports.updateForm = async (req, res) => {
  try {
    ...
    let result = await Form.findByIdAndUpdate(   //update the Form
                id, 
