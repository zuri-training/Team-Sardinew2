...
exports.getUserById = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ success: false, message: "User not Found in the DB!" });
    }
    return res
      .status(200)
  ...
  try {
    ...
    if (user) {
      ...
      Jwt.sign({ result }, jwtKey, { expiresIn: "2h" }, (err, token) => {
        ...
        let htmlContent =
          "<h1 style='font:bold;'><strong>FeedBack Gen</strong></h1>";
        htmlContent += `<p>Hi, ${fname} </p>`;
        htmlContent += "<p>You'registration to feedback Generator.</p>";
        htmlContent +=
          "<p>was successful, Login to start exploring mouth watering offers.</p>";
        ...
        sendEmail(
          user.email,
          "Feedback Gen Successful Registration",
          htmlContent
        );
...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (!checkUser) {
      return res.status(409).json({
        success: false,
        message: "User with this Email doesnt Exist. enter a valid email!",
      });
    }
    ...
    if (!result) {
      return res
        .status(400)
        .json({ success: false, message: "User failed to Update!" });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Error occurred while trying to Update User",
      error: error.message,
    });
  }
};
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    if (!result) {
      return res
        .status(400)
        .json({ success: false, message: "Attempt to delete User failed!" });
    }
    return res.status(200).json({
      success: true,
...
exports.login = async (req, res) => {
  try {
    ...
    if (user && (await bycrypt.compare(password, user.password))) {
      //validate and compare the two passwords
      result = user.toObject(); //remove the password from saving in session or returning
      delete result.password;
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    htmlContent +=
      "<p>You're almost ready to access our feedback Generator.</p>";
    htmlContent +=
      "<p>Simply tap on the big blue button below to verify your account.</p>";
    htmlContent +=
      '<p><a href="' +
      link +
      '" style="color:white;border-radius:10px;background-color: #008CBA; padding:15px 32px; text-align:center; text-decoration:none; display:inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer;"> Verify Account </a></p>';
    htmlContent += "<p>See you inside!</p>";
    htmlContent += "<p><Regards,</p>";
    htmlContent += "<p>Sardinew2 Team</p>";
    ...
    return res.status(200).json({
      success: true,
      message: "Verification email has been sent to you Successfully",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Error occurred while trying to send Email",
      error: error.message,
    });
  }
};
